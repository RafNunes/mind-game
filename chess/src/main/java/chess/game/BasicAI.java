 ...
 * 
 * @author craigmartin
 *         Basic AI. Current AI should be able to beat. Currently
 *         Draws/Stalemates
 * 
 */
...
public class BasicAI implements AI {
	...
	public Move makeMove(Board board) {
		...
		MoveValuePair pair = getBestMove(board, 0, maxVal);
		return pair.move;
	}
	...
	private MoveValuePair getBestMove(Board board, int currentDepth, int upperBound) {
		...
		if (currentDepth >= maxDepth) {
			...
			// For now, we don't like stalemate. In the future, this should
			// depend on how strong a position we are in.
			if (moves.isEmpty())
				return new MoveValuePair(null, -maxVal);
			else {

				// make a list of moveValuePair's for each move, assign a very
				// cheap and rough value to each, and order them accordingly
				...
				for (MoveValuePair mvp : moveValuePairs) {
					...
					// if the best counter that the opponent can make to this
					// move is better for us then the previously best move
	...
	// currently the value is calculated from scratch every time, can we carry
	// this across?
	...
	/**
	 * Gets the material difference. Low cost, essential for basic decision
	 * making
	 * 
	 * @param board
	 * @return
	...
	/**
	 * Get the number of available moves. High cost, difficult for exact
	 * numbers, should have
	 * 
	 * @param board
	 * @return
	...
	protected int getMovesValue(Board board) {
		...
		return board.generateMoves().size() * 2 - board.generateMovesFromOpponentsPerspective().size() * 2; // pawn
																											// is
																											// worth
																											// 10
	}
	...
	/**
	 * Created by Craig Martin, improves collection of material - useful for
	 * denoting an endgame
	 * 
	 * @param pawnValue
	 * @param rookValue
